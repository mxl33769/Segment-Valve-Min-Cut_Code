...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 14 13:49:08 2020
...
data= pd.read_excel('wtown_valves_2.xlsx', sheet_name=0, index='segment')
...
location_data= pd.read_excel('wtown_pipes_2.xlsx', sheet_name=0, index='segment')
seg_pipe=location_data.groupby('segment')['pipe'].apply(lambda x: list(x)).to_dict()
seg_valve_2=seg_valve
pipe_start_data= pd.read_excel('wtown_pipe_start.xlsx', sheet_name=0, index='Label')
pipe_start_dict=pipe_start_data.set_index('Label').T.to_dict('list')
pipe_end_data= pd.read_excel('wtown_pipe_end.xlsx', sheet_name=0, index='Label')
